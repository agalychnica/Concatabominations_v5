...
call_fast_concatabomb()
{
# THIS FUNCTION IS WHAT IS CALLED IN PARALLEL
#$1 is the INPUTFILE
#$2 is the concatabomination file
#$3 is KEEPTMP
		...
		$CONCATABOMBPATH/concatabomb_v2 $1 $2 > $1.abomscores.$SUFFIX;
		
		if [ "$3" == "FALSE" ]; then rm -rf $1.abom.* concatabombout.$SUFFIX p_incomp.out.$SUFFIX p_incomp.error.$SUFFIX; fi;
		
}
export -f call_fast_concatabomb;
		...
		##
			if [ "$END" -ge 7 ]; then  # We need the value of origscore to be calulated if we are going to run any steps after step 7
				...
				if [ "$BINARY" == "FALSE" ]; then  # USE COMPASS
					...
					# We don;t need to do any preprocessing with the new concatabomination approach (i.e. the binary approach).
...
/g' | sed 's/^ *//g' > lk;
				...
				else  # USE FAST METHOD FOR BINARY DATA
				
					si=`wc -l commands.filtered.txt | awk -v NUMPROC=$NUMPROC '{ print ($1/NUMPROC)+1}' | cut -d'.' -f1` #This determines how many parts to split the file into for parallelisation
					echo "si = $si"
					cut -d' ' -f1,2 commands.filtered.txt > commands.filtered.txt.1
					split -l $si commands.filtered.txt.1 commands.filtered.split.
					ls commands.filtered.split.* | xargs -P $NUMPROC -I % bash -c 'call_fast_concatabomb $INFILE % $KEEPTMP';
					cat $INFILE.abomscores.* > $INFILE.abomscores;
					origscore=`cut -d' ' -f2 $INFILE.abomscores | head -1`;
				fi

	
				if [ "$KEEPTMP" == "FALSE" ]; then rm -rf $INFILE.abomscores.* split.* lk compassout compasserror catlist commands.filtered.txt commands.txt commands.filtered.txt.1 commands.filtered.split.*; fi;
			fi
		##
